...
export async function POST(request: NextRequest) {
  try {
    ...
    const { videoData: videoDataRaw, email, anonymousId } = body;
    const videoData: VideoData = videoDataRaw ?? body;
    ...
    // Never trust the client — always re-check subscription state from the DB.
    const { checkSubscription, recordUsage } = await import("@/lib/subscription");
    const subCheck = await checkSubscription(email, anonymousId);
    if (!subCheck.ok) {
      return NextResponse.json(
        {
          error: subCheck.error?.message ?? "Subscription check failed",
          code: subCheck.error?.code,
          plan: subCheck.plan,
          analysesUsed: subCheck.analysesUsed,
          analysesLimit: subCheck.analysesLimit,
        },
        { status: subCheck.error?.httpStatus ?? 403 }
      );
    }
    const isPro = subCheck.plan === "pro";
    ...
    // Try to use AI for enhanced insights (Pro-only — Free gets local insights only)
    ...
    const analysisResult = {
      ...
      // Free users: hide competitor analysis, SEO suggestions, and advanced AI insights
      bestDays,
      bestTimes,
      contentSuggestions: isPro ? contentSuggestions : null,
      // Pro-only flags — frontend reads these to gate UI sections
      isPro,
      plan: subCheck.plan,
      watermark: !isPro, // Free: show "Generated by VideoIQ Free" watermark
    ...
    // Record usage AFTER successful analysis (non-blocking).
    await recordUsage(subCheck, videoData.videoUrl ?? `https://youtu.be/${videoData.videoId}`, videoData.videoId);
