...
export function PaymentModal() {
  ...
  const [localEmail, setLocalEmail] = useState(userEmail || (typeof window !== "undefined" ? window.localStorage.getItem("videoiq:userEmail") || "" : ""));
  const [localName, setLocalName] = useState(userName || (typeof window !== "undefined" ? window.localStorage.getItem("videoiq:userName") || "" : ""));
  ...
  const handlePayment = async () => {
    ...
    // Persist to localStorage so the Navbar + useSubscription can pick it up
    // on subsequent renders / reloads (Free users who paid get recognized).
    if (typeof window !== "undefined") {
      try {
        if (localEmail) window.localStorage.setItem("videoiq:userEmail", localEmail);
        if (localName) window.localStorage.setItem("videoiq:userName", localName);
      } catch { /* ignore */ }
    }
    ...
    try {
      ...
      if (orderData.demoMode) {
        ...
        try {
          ...
          if (verifyRes.ok && verifyData.success) {
            // Refresh subscription state in the rest of the app.
            try {
              const subRes = await fetch(`/api/subscription?email=${encodeURIComponent(localEmail)}`);
              const subData = await subRes.json();
              if (subRes.ok) setSubscription(subData);
            } catch (subErr) {
              console.warn("[payment-modal] demo subscription refresh failed:", subErr);
            }
          ...
          setError(
            verifyData?.error ||
            `Activation failed (HTTP ${verifyRes.status}). reqId=${verifyData?.reqId || "unknown"}`
          );
        } catch (networkErr) {
          setError(
            `Cannot reach activation server. (${networkErr instanceof Error ? networkErr.message : "network error"})`
          );
        }
        setIsProcessing(false);
        return;
      ...
      if (typeof window !== "undefined" && (window as any).Razorpay) {
          ...
          handler: async function (response: any) {
            try {
              ...
              try {
                ...
                // Always try to parse JSON, even on non-2xx
                try { verifyData = await verifyRes.json(); } catch { verifyData = {}; }

                if (verifyRes.ok && verifyData.success) {
                  // Refresh subscription state so the rest of the app sees Pro immediately.
                  try {
                    const subRes = await fetch(`/api/subscription?email=${encodeURIComponent(localEmail)}`);
                    const subData = await subRes.json();
                    if (subRes.ok) setSubscription(subData);
                  } catch (subErr) {
                    console.warn("[payment-modal] subscription refresh failed:", subErr);
                  }
                ...
                // Surface the EXACT backend error (with reqId for support).
                const backendError =
                  verifyData?.error ||
                  `Activation failed (HTTP ${verifyRes.status}). Please contact support with request id: ${verifyData?.reqId || "unknown"}`;
                setError(backendError);
                setIsProcessing(false);
              } catch (networkErr) {
                console.error("[payment-modal] verify network error:", networkErr);
                setError(
                  `Cannot reach activation server. Check your connection and contact support if your card was charged. (${networkErr instanceof Error ? networkErr.message : "network error"})`
                );
                setIsProcessing(false);
              }
            } catch (outerErr) {
              console.error("[payment-modal] handler error:", outerErr);
              setError("Payment verification failed unexpectedly. Please contact support if your card was charged.");
              setIsProcessing(false);
            }
          },
          modal: {
        ...
        rzp.on("payment.failed", function (resp: any) {
          ...
          setError(
            reason
              ? `Payment failed: ${reason}${code ? ` (code ${code})` : ""}`
          ...
          >
            ...
            {paymentSuccess ? (
              ...
              <div className="p-8 text-center">
                ...
                <h3 className="text-2xl font-bold mb-2">Welcome to VideoIQ Pro</h3>
