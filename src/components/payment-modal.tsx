...
export function PaymentModal() {
  ...
  const handlePayment = async () => {
    ...
    try {
      // Step 1: Create Razorpay order via /api/create-order
      ...
      // Try to parse JSON; fall back to a meaningful message if parsing fails
      let orderData: any = null;
      const rawText = await orderRes.text();
      try {
        orderData = JSON.parse(rawText);
      } catch {
        setError(`Server returned ${orderRes.status}. Please try again.`);
        setIsProcessing(false);
        return;
      }

      if (!orderRes.ok || !orderData?.orderId) {
        // Surface the EXACT backend error — never the generic "Failed to create payment order"
        const backendError =
          orderData?.error ||
          orderData?.message ||
          `Failed to create payment order (HTTP ${orderRes.status}). Please try again.`;
        console.error("[create-order] Backend error:", {
      ...
      console.log("[create-order] Order created:", orderData);

      // Demo mode (no real Razorpay keys configured on server)
      ...
      const keyId = orderData.keyId || (process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID as string | undefined);
      if (!keyId) {
        ...
        handler: async function (response: any) {
          try {
            ...
            if (verifyRes.ok && verifyData.success) {
              ...
              console.error("[verify-payment] Backend error:", verifyData);
              setError(
                verifyData.error ||
                  `Payment verification failed (HTTP ${verifyRes.status}). Please contact support with payment ID: ${response.razorpay_payment_id}`
              );
              setIsProcessing(false);
            }
          } catch (verifyErr) {
            console.error("[verify-payment] Network error:", verifyErr);
            setError("Could not reach the verification server. Please contact support.");
            setIsProcessing(false);
          }
        },
        modal: {
      ...
      try {
        ...
        rzp.on("payment.failed", function (resp: any) {
          const reason = resp?.error?.description || resp?.error?.reason || "unknown reason";
          const code = resp?.error?.code ? ` [${resp.error.code}]` : "";
          setError(`Payment failed${code}: ${reason}`);
          setIsProcessing(false);
        });
        rzp.open();
      } catch (openErr) {
        console.error("[razorpay.open] Error:", openErr);
        setError("Could not open Razorpay checkout. Please refresh and try again.");
        setIsProcessing(false);
      }
    } catch (networkErr) {
