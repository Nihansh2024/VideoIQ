    ...
    <section id="pricing" className="py-20 sm:py-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {plans.map((plan, i) => {
            const isCurrentFreePlan = plan.name === "Free" && !isPro;
            const isCurrentProPlan = plan.name === "Pro" && isPro;

            // For Free users, the "Free" card is the current plan.
            // For Pro users, the "Pro" card is the current plan.
            const isCurrentPlan = isCurrentFreePlan || isCurrentProPlan;
            ...
            return (
              ...
              >
                {plan.highlighted && !isCurrentProPlan && (
                ...
                {isCurrentPlan && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-gradient-to-r from-green-500 to-emerald-500 text-white text-xs font-bold flex items-center gap-1">
                ...
                {/* Pro plan: show renewal date when active */}
                {isCurrentProPlan && endDateStr && (
                  <div className="mb-4 p-2.5 rounded-lg bg-brand/5 border border-brand/20 text-xs text-center">
                    <span className="text-muted-foreground">Renews on </span>
                    <span className="font-semibold text-brand">{endDateStr}</span>
                  </div>
                )}
